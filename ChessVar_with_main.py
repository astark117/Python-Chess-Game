...
class ChessVar:
    ...
    def change_game_state(self):
        ...
        if self._curr_player.get_color() == 'white':
            ...
            raise GameOver("This game is over.")
        else:
...
class GameOver(Exception):
    """
    error handling to end the game after one player wins
    """
    pass
...
class InvalidValue(Exception):
    """
    error handling to manage invalid entries.
    """
    pass
...
def main():
    ...
    while True:
        ...
        print("It is now the " + game.get_current_player().get_color() + " player's turn.")

        # Get player input for the move or inserting a fairy piece
        action = input("Enter 'move' to make a move or 'fairy' to insert a fairy piece: ")
        ...
        if action == 'move':
            ...
            # Make the move
            move_result = game.make_move(from_square, to_square)

            if move_result == False:
                print("Invalid move. Try again.")
                continue
        elif action == 'fairy':
            piece_symbol = input("Enter the symbol of the fairy piece ('F' for Falcon, 'H' for Hunter): ")
            square_str = input("Enter the square you want to place the fairy piece on: ")

            # Insert the fairy piece
            fairy_result = game.enter_fairy_piece(piece_symbol, square_str)

            if fairy_result == False:
                print("Invalid fairy piece insertion. Try again.")
                continue
        else:
            print("Invalid action. Please enter 'move' or 'fairy'.")
            continue

        # Check if the game has ended
        game_state = game.get_game_state()
        if game_state != 'UNFINISHED':
            print("Game Over. Result:", game_state)
            break
